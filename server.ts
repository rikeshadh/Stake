...
app.get("/api/agent/signals", async (req, res) => {
  ...
  try {
    const user = await getUserRecord(email);
    const watchlist = (user.watchlist && Array.isArray(user.watchlist)) ? user.watchlist : [];
...
app.post("/api/agent/strategy-analysis", async (req, res) => {
  const body = req.body || {};
  const nested = (typeof body.symbol === "object" && body.symbol !== null) ? body.symbol : {};
  const strategy = body.strategy || nested.strategy || "dip_buyer";
  const profile = body.profile || nested.profile || "balanced";
  const timeframe = body.timeframe || nested.timeframe || "1mo";
  const email = body.email || nested.email;
  const singleSymbol = (typeof body.symbol === "string" ? body.symbol : undefined) || body.ticker;
  ...
  try {
    ...
    if (singleSymbol && !watchlist.includes(singleSymbol.toUpperCase())) {
      watchlist.unshift(singleSymbol.toUpperCase());
    }
    ...
    const promptText = singleSymbol
      ? `Provide a concise, high-conviction quantitative institutional equity & strategy analysis for ${singleSymbol.toUpperCase()} under the "${strategy}" algorithm (${profile.toUpperCase()} profile) on Stake AI.
Context:
...
Please analyze:
1. ${singleSymbol.toUpperCase()} Tactical Valuation & Regime Health (Bullish, Mean-Reverting, or Defensive)
2. Alpha vs S&P 500 Benchmark (estimated basis points outperformance)
3. Volatility & Maximum Drawdown risk mitigation
4. Top 3 Recommended Tactical Actions for the autonomous agent regarding ${singleSymbol.toUpperCase()}.`
      : `Provide a concise, high-conviction quantitative institutional strategy analysis for the "${strategy}" algorithm (${profile.toUpperCase()} profile) deployed on Stake AI.
Context:
- Current Market Prices: ${validQuotes.map((q) => `${q.ticker}: $${q.price} (${q.changePercent >= 0 ? "+" : ""}${q.changePercent}%)`).join(", ")}
- User Portfolio: Cash: $${portfolioSummary.cash}, Positions: ${portfolioSummary.holdings.map((h) => `${h.shares}x ${h.ticker}`).join(", ") || "None"}
- Benchmark Comparison: S&P 500 (SPY) 30-Day Trend is +2.1%
